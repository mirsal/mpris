...
-->
  ...
  <xsl:template match="tp:docstring">
    <xsl:choose>
      <xsl:when test="docbook:para or docbook:formalpara or docbook:simpara">
  ...
  <xsl:template match="property" mode="detail">
    ...
    <refsection>
      ...
      <xsl:apply-templates select="tp:docstring"/>
      <xsl:if test="annotation[@name='org.freedesktop.DBus.Property.EmitsChangedSignal'][@value='true'] or (not(annotation[@name='org.freedesktop.DBus.Property.EmitsChangedSignal'][@value='false']) and ../annotation[@name='org.freedesktop.DBus.Property.EmitsChangedSignal'][@value='true'])">
        <note><para>When this property changes, the <literal>org.freedesktop.DBus.Properties.PropertiesChanged</literal> signal is emitted with the new value.</para></note>
      </xsl:if>
      <xsl:apply-templates select="tp:added"/>
      <xsl:apply-templates select="tp:changed"/>
      <xsl:apply-templates select="tp:deprecated"/>
  ...
  <xsl:template match="text()">
    <xsl:choose>
      <xsl:when test="ancestor::tp:docstring">
        <xsl:value-of select="." />
      </xsl:when>
    ...
    <xsl:message terminate="yes">
      <xsl:choose>
        <xsl:when test="namespace-uri(.) = ''">
          ...
          <xsl:text>&#8221; with no namespace (did you forget an xmlns attribute?)&#10;</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>Unrecognised element: {</xsl:text>
